...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.Uint64Flag{
			Name:        "stratum_min_difficulty, smind",
			Value:       1,
			Usage:       "stratum min difficulty",
			EnvVar:      "STRATUM_MIN_DIFFICULTY",
			Destination: &stratumConfig.MinDifficulty,
		},
		cli.Uint64Flag{
			Name:        "stratum_max_difficulty, smaxd",
			Value:       4611686018427387904,
			Usage:       "stratum max difficulty",
			EnvVar:      "STRATUM_MAX_DIFFICULTY",
			Destination: &stratumConfig.MaxDifficulty,
		},
		cli.Float64Flag{
			Name:        "stratum_difficulty_factor, sjdf",
			Value:       10,
		...
		cli.Uint64Flag{
			Name:        "stratum_default_difficulty, sdd",
			Value:       16384,
			Usage:       "stratum default difficulty",
			EnvVar:      "STRATUM_DEFAULT_DIFFICULTY",
			Destination: &stratumConfig.DefaultDifficulty,
		},
		cli.StringFlag{
			Name:        "stratum_default_difficulty_file_path, sddfp",
			Value:       DefaultDifficultyFilePath,
			Usage:       "stratum default difficulty file path",
		...
		cli.StringFlag{
			Name:        "stratum_subscribe_difficulty_placeholder, ssdp",
		...
		cli.IntFlag{
			Name:        "stratum_subscribe_extra_nonce1_length, ssen1l",
			Value:       8,
			Usage:       "stratum subscribe extra nonce1 length",
			EnvVar:      "STRATUM_SUBSCRIBE_EXTRA_NONCE1_LENGTH",
			Destination: &stratumConfig.ExtraNonce1Length,
		},
		cli.IntFlag{
			Name:        "stratum_subscribe_extra_nonce2_length, ssen2l",
			Value:       4,
			Usage:       "stratum subscribe extra nocne2 length",
			EnvVar:      "STRATUM_SUBSCRIBE_EXTRA_NONCE2_LENGTH",
			Destination: &stratumConfig.ExtraNonce2Length,
		},
		cli.BoolFlag{
			Name:        "stratum_authorize_skip_verify, sakv",
			EnvVar:      "STRATUM_AUTHORIZE_SKIP_VERIFY",
			Usage:       "stratum authorize skip verify",
			Destination: &stratumConfig.SkipVerify,
		},
		cli.StringFlag{
			Name:        "stratum_authorize_name_separator, sans",
			EnvVar:      "STRATUM_AUTHORIZE_NAME_SEPARATOR",
			Value:       ".",
			Usage:       "stratum_authorize_name_separator",
			Destination: &stratumConfig.NameSeparator,
		},
		cli.StringFlag{
			Name:        "stratum_notify_coin_type, snct",
		...
		cli.StringFlag{
			Name:        "stratum_notify_coin_tag, snctag",
		...
		cli.StringFlag{
			Name:        "stratum_notify_payout_address, snpa",
			EnvVar:      "STRATUM_NOTIFY_PAYOUT_ADDRESS",
			Value:       "1K3kCJFD2PYF99t2eBSQ3stmdA1jrp7Nyf",
			Usage:       "stratum notify payout address",
			Destination: &stratumConfig.PayoutAddress,
		},
		cli.DurationFlag{
			Name:        "stratum_notify_loop_interval, snli",
			EnvVar:      "STRATUM_NOTIFY_LOOP_INTERVAL",
			Value:       25,
			Usage:       "stratum notify loop interval (second)",
			Destination: &stratumConfig.NotifyLoopInterval,
		},
		cli.BoolFlag{
			Name:        "stratum_is_used_test_net, siutn",
			EnvVar:      "STRATUM_IS_USED_TEST_NET",
			Usage:       "stratram is used test net",
			Destination: &stratumConfig.UsedTestNet,
		},
		cli.DurationFlag{
			Name:        "stratum_difficulty_check_loop_interval, sdcli",
			EnvVar:      "STRATUM_DIFFICULTY_CHECK_LOOP_INTERVAL",
			Value:       60,
			Usage:       "stratum difficulty check loop interval (second)",
			Destination: &stratumConfig.DifficultyCheckLoopInterval,
		},
		cli.DurationFlag{
			Name:        "stratum_job_hash_check_loop_interval, sjhcli",
			EnvVar:      "STRATUM_JOB_HASH_CHECK_LOOP_INTERVAL",
			Value:       1000,
			Usage:       "stratum job hash check loop interval (Millisecond)",
			Destination: &stratumConfig.JobHashCheckLoopInterval,
		},
		cli.StringFlag{
			Name:        "stratum_sphere_grpc_server_host, ssgsh",
			EnvVar:      "STRATUM_SPHERE_GRPC_SERVER_HOST",
			Value:       "localhost",
			Usage:       "stratum_sphere_grpc_server_host",
			Destination: &sphereGRPCConfig.Host,
		},
		cli.IntFlag{
			Name:        "stratum_sphere_grpc_server_port, ssgsp",
			EnvVar:      "STRATUM_SPHERE_GRPC_SERVER_PORT",
			Value:       80,
			Usage:       "stratum sphere grpc server port",
		...
		cli.StringFlag{
			...
			Value:       "localhost",
			Usage:       "stratum_stats_grpc_server_host",
		...
		cli.IntFlag{
			...
			Value:       8080,
			Usage:       "stratum_stats_grpc_server_port",
