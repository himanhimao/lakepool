...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.StringFlag{
			...
			EnvVar:      "STATS_PUSHER_DB_MEASUREMENT_PREFIX",
		...
		cli.StringFlag{
			...
			EnvVar:      "STATS_PUSHER_DB_MEASUREMENT_SUFFIX",
			Destination: &pusherConfig.MeasurementSuffix,
		},
		cli.StringFlag{
			Name:        "db_measurement_query_precision",
			Value:       "m",
			Usage:       "influx db measurement query precision",
			EnvVar:      "STATS_PUSHER_DB_MEASUREMENT_PRECISION",
		...
		cli.StringFlag{
			...
			EnvVar:      "STATS_PUSHER_DB_MEASUREMENT_RETENTION_STRATEGY",
